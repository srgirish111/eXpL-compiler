...
void typecheck(struct tnode *t)
{
    ...
    case assg_node:
        ...
        if(t->left->type==NULL)
        {
            if(t->left->Ctype !=t->right->Ctype && t->left->Ctype!=t->right->Ctype->Parentptr)
            {
...
void check_name_equi(struct Typetable *type,char *name,struct param *p)
{
    ...
    else
    {
        ...
        if(mtemp->isdefined!=0 && mtemp->isinherited ==0)
        {
            //printf("%s :multiple definitions of functions found in class -->%s\n",name,Cptr->name);
            printf("%d %d %s\n",mtemp->isdefined,mtemp->isinherited,name);
            exit(1);
        }
        mtemp->isdefined++;
...
int codegen(struct tnode* t)
{
    ...
    switch(t->nodetype)
	{
        ...
        case assg_node:
            ...
            freereg();
			freereg();
            if(t->left->Ctype!=NULL &&t->right->nodetype==new_node)
            {
                fp=fopen("task.xsm","a");
                fprintf(fp,"MOV R0,%d\n",t->left->Gentry->binding+1);
                fprintf(fp,"MOV R1,%d\n",4096+8*(t->left->Ctype->Class_index));
                fprintf(fp,"MOV [R0],R1\n");
                fclose(fp);
            }
            else if(t->left->Ctype!=NULL &&t->left->Ctype!=NULL &&t->left->nodetype==var_node &&t->right->nodetype==var_node)
            {
                fp=fopen("task.xsm","a");
                fprintf(fp,"MOV R0,%d\n",t->left->Gentry->binding+1);
                fprintf(fp,"MOV R1,%d\n",t->right->Gentry->binding+1);
        ...
        case cls_meth_node:
            ...
            i=getreg();
            fp=fopen("task.xsm","a");
            fprintf(fp,"MOV R%d,%d\n",i,t->Gentry->binding+1);
            //printf("%d\n",t->Gentry->binding+1);
            fprintf(fp,"MOV R%d,[R%d]\n",i,i);//vptr
            fprintf(fp,"ADD R%d,%d\n",i,t->mid->val);
            fprintf(fp,"MOV R%d,[R%d]\n",i,i);//address of func in stack
            //printf("%d\n",t->mid->val+1);
            fprintf(fp,"CALL R%d\n",i);
            //fprintf(fp,"CALL F%d\n",t->mid->val);
            fclose(fp);
...
void add_vptr(struct Gsymbol *gsym)
{
    FILE *fp;
    fp=fopen("task.xsm","a");
    fprintf(fp,"MOV R0,%d\n",gsym->binding+1);
    fprintf(fp,"MOV [R0],%d\n",4096 + 8*(gsym->Ctype->Class_index));
    fclose(fp);
}
void main_starter()
{
...
void printVirtualTable()
{
    ...
    FILE *fp = fopen("task.xsm", "a");
	struct Classtable *temp = cls_head;

    while (temp!=NULL)
    {
        int stkloc = 4096 + 8*temp->Class_index;
        int i = 0;
...
void Class_PFinstall(struct Classtable *cptr, struct Fieldlist *field)
{
	...
	{
		printf("There cannot be more than 8 class fields \n");
		exit(1);
	}
	if (Class_Flookup(cptr, field->name) != NULL)
	{
		printf("Class Field already declared\n");
		exit(1);
	}
...
void Class_PMinstall(struct Classtable *cptr, struct Memberfunclist *Mfunc)
{
	...
	{
		printf("There cannot be more than 8 class methods \n");
		exit(1);
	}
	//if (Class_Mlookup(cptr, Mfunc->name) != NULL)
	//{
		//printf("Class member function already declared\n");
		//exit(1);
	//}
...
void checkoverriding(struct Memberfunclist* parent_fn , char *name, struct Typetable *type, struct param *Paramlist){
	if((strcmp(parent_fn->name, name)==0) && (parent_fn->type == type)){
			struct param *parent_param;
            parent_param = parent_fn->paramlist;
			struct param *temp  = Paramlist;
			while(parent_param != NULL && temp != NULL){
				if((parent_param->name != temp->name) || (parent_param->type != temp->type)){
						printf("Function overriding requires signature of both functions to match exactly\n");
						exit(1);
				}
				else{
...
void install_parent (struct Classtable *temp, struct Classtable *parent)
{
...
struct Classtable *CInstall(char *name,char *parent_class_name)
{
    ...
    if(parent_class_name!=NULL)
    {
        ...
        par =CLookup(parent_class_name);
        if(par==NULL)
        {
            printf("no parent class %s\n",parent_class_name);
            exit(1);
        }
        temp->Parentptr= par;
        install_parent(temp,par);
    }
    else
    {
        temp->Parentptr=NULL;
...
void Class_Minstall(struct Classtable *cptr, char *name, struct Typetable *type, struct param *Paramlist,int idx)
{
    ...
    if(Class_Mlookup(cptr,name) != NULL)
    {
        ...
        if (cptr->Parentptr==NULL ||Class_Mlookup(cptr->Parentptr, name) == NULL)
		{
            printf("a method with this name--> %s already exists in class-->%s",name,cptr->name);
            exit(1);
        }
        checkoverriding(Class_Mlookup(cptr->Parentptr, name), name, type, Paramlist);
        temp->Flabel=f_no++;
        return;
    }
...
//changes to be made in typecheck as some statemets cant occur in a class
 ...
 //        1   int init();
   //      2   bst getroot();
     //    3   int setroot(bst n1);
      //   4   bst getnode(int key);
       //  5   bst insert(bst h, int key);
       //  6   int inOrder_fun(bst h);
        // 7   int preOrder_fun(bst h);
        // 8  int postOrder_fun(bst h);